...
pub fn risk(volatility:f64,liquidity:f64,data_quality:f64,timeframe_conflict:f64,anomaly:f64)->RiskResult{let score=(volatility*.25+liquidity*.25+(100.0-data_quality)*.30+timeframe_conflict*.10+anomaly*.10).clamp(0.0,100.0);let level=if score>=80.0{RiskLevel::Extreme}else if score>=60.0{RiskLevel::High}else if score>=30.0{RiskLevel::Medium}else{RiskLevel::Low};RiskResult{score,level}}
...
pub fn signal_quality(evidence:&[f64])->SignalQuality{if evidence.is_empty(){return SignalQuality{score:0.0,agreement:0.0,conflict:100.0,confirmations:0}}let pos=evidence.iter().filter(|x|**x>0.0).count();let neg=evidence.iter().filter(|x|**x<0.0).count();let majority=pos.max(neg);let agreement=majority as f64/evidence.len() as f64*100.0;let conflict=100.0-agreement;let magnitude=evidence.iter().map(|x|x.abs().min(100.0)).sum::<f64>()/evidence.len() as f64;SignalQuality{score:(agreement*.7+magnitude*.3).clamp(0.0,100.0),agreement,conflict,confirmations:majority as u8}}
