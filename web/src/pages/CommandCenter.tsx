    ...
    <div className="page">
      ...
      {data && <>
        <section className="metric-grid">
          ...
          <article><span>{t.colConfidence}</span><strong>{data.confidence.toFixed(1)}%</strong><small>{t.ccConfidenceSub}</small></article>
        ...
        <section className="panel">
          ...
          <div className="market-table">
            <div className="market-row table-head"><span>{t.colMarket}</span><span>{t.colPrice}</span><span>{t.colDecision}</span><span>{t.colConfidence}</span><span>{t.colRisk}</span><span>{t.colRegime}</span></div>
        ...
        <div className="dashboard-split">
          ...
          <section className="panel"><span className="eyebrow">{t.ccLatestIntelligence}</span><h2>{t.ccWhyPrefix} {data.decision}?</h2><ul className="reason-list">{data.reasons.length ? data.reasons.map((r, i) => <li key={i}>{r}</li>) : <li>{t.ccNoExplanation}</li>}</ul><div className="confidence-block"><span>{t.ccMetaConfidence}</span><strong>{data.confidence.toFixed(1)}%</strong></div></section>
        </div>
      </>}
    </div>
  );
