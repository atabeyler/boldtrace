...
export function MarketScanner() {
  ...
  return <Page t={t} title={t.scannerTitle} sub={t.scannerSub}>
    ...
    {data && data.length > 0 && <section className="panel"><div className="engine-row engine-head"><span>{t.colMarket}</span><span>{t.colDecision}</span><span>{t.colConfidence}</span><span>{t.colRisk}</span><span>{t.colStatus}</span></div>{data.map(x => <div className="engine-row responsive-data-row" key={x.symbol}><strong data-label={t.colMarket}>{x.symbol}</strong><b data-label={t.colDecision}>{x.market?.decision ?? '—'}</b><span data-label={t.colConfidence}>{x.market ? `${x.market.confidence.toFixed(0)}%` : '—'}</span><span data-label={t.colRisk}>{x.market ? `${x.market.risk.toFixed(0)}%` : '—'}</span><span data-label={t.colStatus} className={x.status === 'live' ? 'healthy' : 'stale'}>{x.status === 'live' ? `● ${t.statusLive}` : x.status === 'stale' ? `○ ${t.itDataStale}` : `○ ${t.statusUnavailable}`}</span></div>)}</section>}
  </Page>;
}
